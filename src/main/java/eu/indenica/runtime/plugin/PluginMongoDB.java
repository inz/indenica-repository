...
public class PluginMongoDB implements IPlugin {
    ...
    public Data storeData(Data d) {
        LOG.debug("Storing data in MongoDB");
    ...
    public Data getData(Filter f) {
        try {
            ...
            String query =
                    util.xml.getChildElements(((Element) f.value), "query")
                            .get(0).getTextContent();
            LOG.debug("Finding data in MongoDB: " + query);
            ...
            while(c.hasNext()) {
                DBObject o = c.next();
                util.xml.appendChild((Element) d.value,
                        util.xml.toElement(com.mongodb.util.JSON.serialize(o)));
            }
